...
const EquipmentAdd: React.FC = () => {
    ...
    const { loading, error } = useSelector((state: RootState) => state.equipment);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (validateForm()) {
            ...
            try {
                const resultAction = await dispatch(addEquipment(processedData)).unwrap();
                toast.success('Équipement ajouté avec succès!');
                // Redirect to equipment detail page after successful creation
                setTimeout(() => {
                    navigate(`/equipments/${resultAction._id}`);
                }, 2000);
            } catch (err) {
                ...
                <div className="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="flex justify-end space-x-4 pt-4">
                            ...
                            >
                                {loading || isSubmitting ? 'Envoi en cours...' : 'Ajouter l\'équipement'}
                            </button>
                        </div>
                    </form>
                </div>
