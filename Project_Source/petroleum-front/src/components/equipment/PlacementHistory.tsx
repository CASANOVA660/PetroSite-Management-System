...
const PlacementHistory: React.FC<PlacementHistoryProps> = ({ history, isLoading }) => {
    ...
    // Filter history entries by type "placement"
    const placementHistory = history.filter(entry => entry.type === 'placement');

    useEffect(() => {
        let filtered = [...placementHistory];
    ...
    }, [placementHistory, dateFilter, startDate, endDate, searchTerm]);
    ...
    return (
        <div className="space-y-4">
            ...
            {isLoading ? (
                ...
                <EmptyHistoryState
                    message="Aucun enregistrement de placement n'a été trouvé pour cet équipement."
                />
            )}
        </div>
    );
